...
def shift_right(x, pad_value):
    """
    Shift the input tensor to the right by padding on the left and truncating on the right.
    :param x: A 2D tensor where each row is a sequence.
    :param pad_value: The padding value to use (e.g., index of BOS token).
    :return: The shifted tensor.
    """
    # Create a tensor of the same size filled with pad_value
    pad_tensor = torch.full_like(x[:, :1], pad_value)

    # Concatenate the pad tensor to the left of the input tensor
    # and truncate the last element of each sequence on the right
    shifted_x = torch.cat([pad_tensor, x[:, :-1]], dim=1)
...
class PredictionModel(nn.Module):
	...
	def forward(self, *inputs):
		with torch.set_grad_enabled(self.fine_tuning):
			all_features = []
			for ipt in inputs:
				# Calculate src_lengths for each input (example for padded sequences)
				src_lengths = torch.sum(ipt.ne(self.task.source_dictionary.pad_index), dim=1)

				# Determine prev_output_tokens (you'll need to adapt this part)
				# Example: Assuming ipt is shifted right (for seq-to-seq models)
				prev_output_tokens = None
				prev_output_tokens = shift_right(ipt, self.task.target_dictionary.pad_index)

				state, features_dict = self.model.extract_features(ipt, src_lengths=src_lengths, prev_output_tokens=prev_output_tokens)
				# all_features.append(features_dict['inner_states'][self.extracted_layer].transpose(0,1))
				feats = self.model.encoder(ipt, src_lengths=src_lengths)['encoder_out'][0].transpose(0,1)
				feats.masked_fill_((ipt == self.task.source_dictionary.pad_index).unsqueeze(-1), 0)
...
parser.add_argument('--model_d', type=int, default=768)
...
def collate_fn(items):
	...
	if args.sentence_level:
		...
		# targets = torch.cat([targets, torch.full((targets.size(0), 128 - targets.size(1)), -100)], dim=1)
		targets = [tgt[:128] for tgt in targets]
...
for epoch in tqdm.trange(args.epochs, desc='epochs'):
	...
	for batch in pbar:
		...
		preds = model(*sources)
		loss = criterion(preds.transpose(1, 2), target)
		loss.backward()
